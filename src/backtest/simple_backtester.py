"""Simple backtester for Yahoo Finance equity data."""
...
class SimpleBacktester:
        ...
        for i, row in df.iterrows():
            ...
            if position == 0 and signal != 0:
                ...
                if shares < self.config.min_trade_size:
                    ...
                    continue
                cash -= cost
                logger.debug(f"Enter {position} {shares} shares at {entry_price:.2f} on {entry_date}")
                ...
                cash += proceeds
                pnl = proceeds - shares * entry_price - 2 * shares * self.config.commission_per_share
                ...
                logger.debug(f"Exit {position} {shares} shares at {exit_price:.2f} on {exit_date}, PnL={pnl:.2f}")
                position = 0
                entry_price = 0.0
                shares = 0
    ...
    def get_equity_curve_df(self) -> pd.DataFrame:
        ...
        return self.equity_curve.reset_index().rename(
            columns={"index": "timestamp", 0: "equity"}
        )
